...
# More Test stacks.
    temp = Stack(3112)
    temp.militaryunits = [ session.query(MilitaryUnit).filter_by(id = 1).one(),
                           session.query(MilitaryUnit).filter_by(id = 3).one()]
    session.add(temp)
    temp = Stack(3112)
    temp.militaryunits = [ session.query(MilitaryUnit).filter_by(id = 4).one(),
                           session.query(MilitaryUnit).filter_by(id = 6).one()]
    session.add(temp)
    session.commit()
